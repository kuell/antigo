 ...
 */
   ...
   function loaded(){
    ...
    tcm = new Menu(120, this);
    tcm.addMenuItem("&nbsp;View Rows", "top.tabset('sqlresults', 1, 'select * from ' + myQuery.tables.item(currentDiv[0].id).table_name)");
    tcm.addMenuItem("&nbsp;Duplicate Table", "duplicateTable(currentDiv[0].getAttribute('tableName'));");
    tcm.addMenuItem("&nbsp;Add Related Tables", "addRelatedTables();");
    tcm.addMenuItem("&nbsp;Change Alias", "changeTableAlias(currentDiv[0].id);");
    tcm.addMenuItem("&nbsp;Remove", "delTables();");
    tcm.addMenuItem("&nbsp;Synchronize", "syncTable();");
    
    lcm = new Menu(100, this);
    lcm.addMenuItem("&nbsp;Edit", "editLink()");
    lcm.addMenuItem("&nbsp;Unlock Position", "unLockLink()");
    lcm.addMenuItem("&nbsp;Remove", "delLink();");
    
    rcm = new Menu(100, this);
    rcm.addMenuItem("&nbsp;Save query", "parent.showPleaseWait('Please wait while saving', '160px', '');setTimeout('parent.save(1)', 60);");
    rcm.addMenuItem("&nbsp;Run query", "tabset('sqlresults')");
    rcm.addMenuItem("&nbsp;View source", "zdbg(myQuery.serialize())");
