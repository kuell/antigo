...
$ins_procedimento->registerTrigger("END", "Trigger_Default_Redirect", 99, "proc_list.php");
// Add columns
...
$upd_procedimento->registerTrigger("END", "Trigger_Default_Redirect", 99, "proc_list.php");
// Add columns
...
?>
        <table cellpadding="2" cellspacing="0" class="KT_tngtable">
          <tr>
            ...
            <td><div align="left"><select name="ID_AVAL_<?php echo $cnt1; ?>" id="ID_AVAL_<?php echo $cnt1; ?>">
              <option value="" <?php if (!(strcmp("", $row_rsprocedimento['ID_AVAL']))) {echo "selected=\"selected\"";} ?>><?php echo NXT_getResource("Selecione a Avaliação"); ?></option>
              <?php
do {  
?>
              <option value="<?php echo $row_rs_aval['ID_AVAL']?>"<?php if (!(strcmp($row_rs_aval['ID_AVAL'], $row_rsprocedimento['ID_AVAL']))) {echo "selected=\"selected\"";} ?>><?php echo $row_rs_aval['DESC_AVAL']?></option>
              <?php
...
?>
            ...
            <?php echo $tNGs->displayFieldError("procedimento", "ID_AVAL", $cnt1); ?></div></td>
          </tr>
          <tr>
            <td class="KT_th"><label for="PROC_DESC_<?php echo $cnt1; ?>">Procedimento:</label></td>
            <td><input type="text" name="PROC_DESC_<?php echo $cnt1; ?>" id="PROC_DESC_<?php echo $cnt1; ?>" value="<?php echo KT_escapeAttribute($row_rsprocedimento['PROC_DESC']); ?>" size="70" maxlength="50" />
      ...
      // endif Conditional region1
      ?>
          <input type="button" name="KT_Cancel1" value="<?php echo NXT_getResource("Cancel_FB"); ?>" onclick="return UNI_navigateCancel(event, 'proc_list.php')" />
        </div>
      </div>
    </form>
  </div>
