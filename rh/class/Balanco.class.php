...
class Balanco {
	...
	public function getCustoRhDia() {
		...
		while ($val = $rs->fetch_object()) {
			if (!$custoHora[$val->id_setor]) {
				$custoHora[$val->id_setor] = $this->getCustoHora($val->id_setor);
			}
	...
	public function getCustoHora($setor) {
		$i = new Interno();

		$hrTrab        = $this->getItem($setor, 1);
		$remBruta      = $this->getItem($setor, 12);
		$hrTrabInterno = doubleval($i->getHorasTrabalhadasBalanco($setor, $this->datai, $this->dataf));
