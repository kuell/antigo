...
class PDF extends FPDF {
	...
	function Dados() {
		...
		$datai = implode('-', array_reverse(explode('-', $_GET['data'])));
		$dataf = implode('-', array_reverse(explode('-', $_GET['data'])));
		$setor = $_GET['setor'];
		...
		if (empty($balanco->setor)) {
			$this->Cell(190, 5, 'TODOS', 1, 0, 'C', 1);
		} else {
			$this->Cell(190, 5, $balanco->setor->setor, 1, 0, 'C', 1);
		}
