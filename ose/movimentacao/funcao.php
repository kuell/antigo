<?php
	...
	function orcamento(){		
		...
		$sql = 
			...
			";
		mysql_query($sql) or die (mysql_error());
		echo "<div align='center' class='ewHeaderRow'>O Orçamento foi Lançado, a Ordem Externa esta esperando Aprovação!<div>";
		echo "<div align='center' class='ewHeaderRow'><a href = 'class_form.php?id_OSE=$id'>Classificar Ordem de serviço</a><div>";
		}
	function aprovar(){
		$id 	= $_POST['id_OSE'];
		require('../../Connections/conn.php');
	...
	function receber(){
		...
		if(!$nota){
			echo "<script>alert('O campo nota fiscal não pode ser nulo'); history.back(1);</script>";
			}
	...
	function cancelar(){
		echo "<div align='center'>Ação Cancelada!</div>";
		}
	function estornar(){
		$id = $_REQUEST['id'];
			$sql = "update ordem_externa set
