<?php
  ...
  class PDF extends FPDF {
    function Header() {
      ...
      $this->Cell(191,10,"Peri Alimentos Ltda.","LRT",0,"R");
      $this->Ln();
      $this->SetFont("Arial","B",10);
      $this->Cell(191,10,"Fechamento de taxas","LBR",0,"R");
	  
	//  $this->Cell(191,10,$sql,"LBR",0,"R");

	  $this->Ln();
	  $this->SetTextColor(0);
      $this->SetFont("Arial","IB",8);
	  $this->Cell(91,10,"Ref. ao ".$row['cor_cod'].' - '.$row['cor_nome'], 'LBR', '0', 'C');
    ...
    function Dados() {
      ...
      $sql = "Select * from taxaDoc Where corretor = '".$_REQUEST['cor']."' and data between '$data1' and '$data2' group by tipo order by tipo";
...
// Se for tipo do item for animal	
		...
		$this->Cell($w[3],4, 'Unidade',"LRTB",0,"L",$fill);
			...
			$sqlItens = "Select 
								*, 
								sum(peso) as peso, 
								sum(qtd) as qtd,
								(select sum(qtd) from taxaDoc where tipo = '".$res['tipo']."' and corretor = '".$_REQUEST['cor']."' and data between '$data1' and '$data2') as total_qtd,
								(select sum(peso) from taxaDoc where tipo = '".$res['tipo']."' and corretor = '".$_REQUEST['cor']."' and data between '$data1' and '$data2') as total_peso
							from 
								taxaDoc 
							where 
								tipo = '".$res['tipo']."' and
								corretor = '".$_REQUEST['cor']."' and
	...
	Case "ITEM" :
		...
		$sqlItens = "Select 
						...
						sum(valor) as valor ,
						sum(peso) as peso,
						(select SUM(valor)from 	taxaDoc where 	tipo_movimento = 2 and corretor = '".$_REQUEST['cor']."' and item = item1 and data between '$data1' and '$data2') as A_Pagar,
						(select SUM(valor)from 	taxaDoc where	tipo_movimento = 1 and corretor = '".$_REQUEST['cor']."' and item = item1 and data between '$data1' and '$data2') as A_Receber,
						(select sum(valor) from taxaDoc where tipo = '".$res['tipo']."' and corretor = '".$_REQUEST['cor']."' and data between '$data1' and '$data2' and tipo_movimento = 2 ) as totalReceber,
						(select sum(valor) from taxaDoc where tipo = '".$res['tipo']."' and corretor = '".$_REQUEST['cor']."' and data between '$data1' and '$data2' and tipo_movimento = 1 ) as totalPagar
					from 
						taxaDoc 
					where 
					data between '$data1' and '$data2' and
					tipo = '".$res['tipo']."' and corretor = '".$_REQUEST['cor']."' group by item1";
			...
			while($item = mysql_fetch_assoc($query)){
				...
				$this->Cell($w[1],4, "",0,"BT","R",1);
				$this->Cell($w[1],4, "TOTAL",0,"BT","R",1);
	...
	$w=array(80,37,37,37);
		...
		$this->Cell($w[3],4, 'Unidade',"LRTB",0,"L",$fill);
			...
			$sqlItens = "Select 
								*, 
								sum(peso) as peso, 
								sum(qtd) as qtd,
								(select sum(qtd) from taxaDoc where tipo = 'EXPED' and corretor = '".$_REQUEST['cor']."' and data between '$data1' and '$data2') as total_qtd,
								(select sum(peso) from taxaDoc where tipo = 'EXPED' and corretor = '".$_REQUEST['cor']."' and data between '$data1' and '$data2') as total_peso
							from 
								taxaDoc 
							where 
								tipo = '".$res['tipo']."' and
			...
			while($item = mysql_fetch_assoc($query)){
				...
				$this->Cell($w[1],4, $item["qtd"],"LRTB",0,"R",0);
				$this->Cell($w[2],4, number_format($item["peso"], 2, ',', '.'),"LRTB",0,"R",0);
				$this->Cell($w[3],4, $item["unidade"],"LRTB",0,"C",0);
				$this->Ln();
	  ...
	  $this->Cell($w[1],4, "","",0,"R",1);
	  $this->Cell($w[2],4, "","",0,"R",1);
